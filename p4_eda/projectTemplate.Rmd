...
[Report2](http://rstudio-pubs-static.s3.amazonaws.com/80458_5000e31f84df449099a872ccf40747b7.html)
[Report3](https://www.minitab.com/en-us/Published-Articles/Wine-Tasting-by-Numbers--Using-Binary-Logistic-Regression-to-Reveal-the-Preferences-of-Experts/)
...
plotVar <- function(v,d) {
  ...
  suppressMessages(grid.arrange(q1, q2, q3, 
...
suppressMessages(grid.arrange(q1, q2, nrow=2))
```
...
```{r echo=FALSE, Univariate_xform2normality_SO2}
...
```{r echo=FALSE, Univariate_citricacid_boxcox}
...
```{r echo=FALSE, Univariate_citricacid_plots}
...
The Box-Cox transformation in this case, although didactic, does not seem to produce a very convincing result...
...
```{r echo=FALSE, message=FALSE, Bivariate_Plots_ggpairs}
...
From the boxplots in this matrix we easily see that our variable of interest (*quality*) has very pronounced **positive correlation** to *citric acid*, *sulphates* and *alcohol*. It has **negative correlation** to *volatile acidity*, *chlorides*, *density* and *pH*. Its relation to the rest of the input variables seems less clearly defined. 
...
```{r echo=FALSE, Bivariate_Plots_fixedvolac}
...
```{r echo=FALSE, Multivariate_sweetness_quality_facets}
...
```{r echo=FALSE, Multivariate_acidity_alcohol_scatterplot}
...
```{r echo=FALSE, Multivariate_ologit}
...
for (mf in f) {
    ...
    rate.m <- count(w.m[w.m$match==T,]) / count(w.m)
...
The best model (i.e. the one having the best prediction rate) is:

```{r, echo=FALSE, Models_best}
best_model$formula
```
...
We see that the predictive power of these models is far from satisfactory (< 60%). Probably, we can have better results using some machine learning approach---for example, **Random Forests** or **Support Vector Machines (SVM)** come to mind.
